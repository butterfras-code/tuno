...
- Accent expression: on each scheduled accented beat, eyebrows lift 5 SVG pixels and ears dip 3 pixels with a 12° outward droop in both head poses. The expression peaks after the first fifth of a 240 ms envelope (shortened to 70% of the beat at fast tempos), then returns to rest. Unaccented beats and subdivisions do not trigger it. Stop and reduced motion clear the transforms.
